...
class OperatorPropagation(Operator):
    ...
    def reduction_edge(edges, edge_transform, *weights):
        cnt_e = edge_transform.max() + 1#边处于簇位置的种类（簇内、割边）数，edge_transform为每条边的位置类型编号
        ...
        ret = [edges
               ] + [silearn.scatter_sum(w, edge_transform) for w in weights] #根据边处在簇位置的类型把边权加起来，同一个簇中的会double
        return tuple(ret)

    @staticmethod
    def get_edge_transform(edges, identical_flag=False):
        max_id = int(edges[:, 1].max() + 1) #此时的edges是簇下标，max_id是簇数
        bd = 1
        shift = 0
        while bd <= max_id:
            bd = bd << 1
        ...
        edge_hash = (edges[:, 0] << shift) + edges[:, 1]  #边右顶点簇的下标+边左顶点簇下标*2^shift  从二进制看就是左部分为左顶点簇，右部分为右顶点簇
        ...
        if identical_flag:
            ...
            _, transform = torch.unique(edge_hash, return_inverse=True)  #_为独立不重复元素组成张量，transform为原始张量在_上索引，边的左右顶点在的簇一样表示两条边当前在的簇位置相同（簇内、割边）
            return transform
    ...
    def sum_up_multi_edge(edges, *weights, operation_ptrs=None):
        if operation_ptrs is not None:
            ...
            cnt = redu[0].shape[0]#边处于簇内位置的种类数（簇内，不同簇间割边）
            ...
            return OperatorPropagation.reduction_edge(edges, trans, *weights)#返回的是一个元组 边处于簇位置的种类 以及每个种类下的边权和
    ...
    def perform(self,
                ...
                m_scale=None,
                re_compute=True):
        # p: the parameters for parallel Merge Operators
        ...
        while operated_cnt > ter or not merge_all:
            max_operate_cnt = math.ceil((current_num_vertices - min_com) * p)#（当前顶点数-最小簇数）*1 最多需要进行的操作数
            ...
            mx = torch.max(edges)#最大顶点下标，+1就是顶点/簇数
            ...
            if not cache:
                ...
                vst = v1[edges] #每条边的左右顶点/簇的di/vol(a)
            ...
            if contains_self_loops:
                if not cache:
                    ...
                    vx = vst.sum(dim=1) #每一条边连接的两个簇的vol(α)求和 ，每一行求和
                    ...
                    #         g0t = g0[edges[:, 1]]
                    #         g0x = g0s + g0t
                    vin = vst - gst #每一条边连接的两个簇的vol(α)-gα 即簇内部边权之和

                    dH1 = (vin[:, 0]) * torch.log2(
                        vst[:, 0]) + (vin[:, 1]) * torch.log2(vst[:, 1]) - (
                            vin[:, 0] + vin[:, 1]) * torch.log2(vx)  # 左簇的 (vol(α)-gα )*log2 vol(α) +右簇的(vol(β)-gβ )*log2 vol(β)-(vol(α)-gα+vol(β)-gβ )*log2 vol(α+β)
            ...
            trans = torch.arange(edges.max() + 1, device=self.enc.graph.device)#合并前的簇划分(一人一个）
            trans[ids] = trans[idt]
            # if operated_cnt < 10:
            ...
            for i in range(int(lg_merge)):
                ...
                # old = trans[ids]
                trans[ids] = trans[trans[ids]]  #合并完的簇划分
                # ids = torch.nonzero(trans[ids] != old)

            # compress id allocation
            trans = torch.unique(trans, return_inverse=True)[1] #trans中每个在独立不重复张量中索引，即在第几个簇
            ...
            if adj_cover is None:
                ...
                edges = trans[edges] #edges从顶点下标变为簇下标
                edges, trans_prob = OperatorPropagation.sum_up_multi_edge(
                    edges, trans_prob)
