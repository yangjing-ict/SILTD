...
class Partitioning(GraphEncoding):
    ...
    def uncertainty(self, es, et, p):
        ...
        id_et = self.node_id[et] #第二列顶点所在簇 E
        id_es = self.node_id[es] #第一列顶点所在簇 E
        v2 = scatter_sum(self.graph.stationary_dist, self.node_id)  #按簇求di/vol(G),v2=vol(α)/vol(G),v2.sum=1
        v2e = v2[id_es] #第一列顶点所在簇的vol(α)/vol(G)
        flag = id_es != id_et #一条边连接两顶点是否在同一簇
        # print(v1e, v2, flag)
        return uncertainty(v1e / v2e) + flag * uncertainty(v2e / v2.sum())
        # 第一列顶点的 log_2 di/vol(α)  +两顶点不在一个簇 log_2 vol(α)/vol(G)
